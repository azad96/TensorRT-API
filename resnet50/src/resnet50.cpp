...
nvinfer1::ITensor* resnet50(nvinfer1::INetworkDefinition *network, std::map<std::string, nvinfer1::Weights>& weightMap, nvinfer1::ITensor* input, int outputCount){
    nvinfer1::Weights emptywts{nvinfer1::DataType::kFLOAT, nullptr, 0};
    ...
    x = bottleneck(network, weightMap, *x, 256, 64, 1, "layer1.1.");
    x = bottleneck(network, weightMap, *x, 256, 64, 1, "layer1.2.");

    x = bottleneck(network, weightMap, *x, 256, 128, 2, "layer2.0.");
    x = bottleneck(network, weightMap, *x, 512, 128, 1, "layer2.1.");
    x = bottleneck(network, weightMap, *x, 512, 128, 1, "layer2.2.");
    x = bottleneck(network, weightMap, *x, 512, 128, 1, "layer2.3.");

    x = bottleneck(network, weightMap, *x, 512, 256, 2, "layer3.0.");
    x = bottleneck(network, weightMap, *x, 1024, 256, 1, "layer3.1.");
    x = bottleneck(network, weightMap, *x, 1024, 256, 1, "layer3.2.");
    x = bottleneck(network, weightMap, *x, 1024, 256, 1, "layer3.3.");
    x = bottleneck(network, weightMap, *x, 1024, 256, 1, "layer3.4.");
    x = bottleneck(network, weightMap, *x, 1024, 256, 1, "layer3.5.");

    x = bottleneck(network, weightMap, *x, 1024, 512, 2, "layer4.0.");
    x = bottleneck(network, weightMap, *x, 2048, 512, 1, "layer4.1.");
    x = bottleneck(network, weightMap, *x, 2048, 512, 1, "layer4.2.");

    nvinfer1::IPoolingLayer* pool2 = network->addPooling(*x, nvinfer1::PoolingType::kAVERAGE, nvinfer1::DimsHW{7, 7});
    assert(pool2);
